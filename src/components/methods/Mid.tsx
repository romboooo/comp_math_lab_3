...
export default function RightRectangleMethod({
  ...
  useEffect(() => {
    ...
    if (epsilon < 0 || epsilon > 1) {
      setError("epsilon должен быть от нуля до единицы");
      return;
    }
    ...
    try {
      ...
      const rungeError = Math.abs((I1 - I0) / 1);
  ...
  return (
    <div className="method-result">
      <h2>Метод средних прямоугольников:</h2>
      <h3>
        <InlineMath
          math={`\\ ${
      ...
      {latest && (
        <div className="calculation-steps">
          ...
          <div>
            <InlineMath
              math={`\\sum_{i=1}^{n}y_{i+1/2}=${formatNumber(latest.sum, 4)}`}
            />
          </div>
          <div>
            <InlineMath
          ...
          <div className="runge-rule">
            ...
            <InlineMath
              math={`\\delta=\\frac{I_{2n}-I_n}{2^1 - 1}=${formatNumber(
                latest.error,
                8
          ...
          <InlineMath
            math={`h \\cdot \\sum_{i=1}^{n} y_{i+1/2} = ${formatValue(
              (b - a) / latest.n,
              4
      ...
      {isConverged && (
        <div className="final-results">
          ...
          <InlineMath math={`I_{прав}=${formatNumber(result.value, 10)}`} />
          <InlineMath
            math={`\\delta=|I-I_{прав}|=${formatNumber(
              result.delta,
              4
